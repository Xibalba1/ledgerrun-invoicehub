...
LedgerRun is a local-first, AI-assisted invoice ingestion hub for clinical-trial finance operations. Its architecture is intentionally staged: untrusted documents and model output are converted into typed records, enriched with reference data, classified by deterministic policy, and then exposed to reviewers with enough evidence to correct or override the result. The core design principle is that AI performs the first pass end to end, while TypeScript validation and deterministic rules make the outcome auditable.
...
7. **AI pipeline (`server/src/pipeline.ts`, `server/src/llm.ts`)**  
   The pipeline persists after each stage: received, extracting, resolving, matching, deciding, done, or failed. Anthropic calls handle PDF extraction, entity-resolution proposals, and semantic catalog-match proposals. Each response is checked for refusal or truncation, parsed as JSON, and validated before it affects state.

8. **Deterministic validation, matching, and decisioning (`server/src/resolve.ts`, `server/src/match.ts`, `server/src/decide.ts`)**  
   Deterministic code validates model-selected IDs, enforces sponsor-study scope, checks protocol equality, calculates price deltas, applies confidence thresholds, and decides submit versus hold. This makes outcomes replayable and explainable: block exceptions hold the invoice, warnings remain visible, and the exception list is the rationale.
...
An invoice enters through upload or demo seed, is normalized to a PDF, and receives an `InvoiceRecord`. The pipeline extracts metadata and line items, fetches reference data through MCP, validates context, fetches a scoped catalog, validates match proposals, and applies submit-or-hold policy. Each transition is persisted and published over SSE. Reviewers inspect the result in the hub; QC actions update the same record, rerun affected matching and decisioning, and leave an audit trail.
