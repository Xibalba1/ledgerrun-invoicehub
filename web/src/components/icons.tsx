...
export function Escalate({ size = 12, ...p }: IconProps) {
  return (
    <svg {...base(size, p)} fill="none">
      <path d="M6 18L18 6M18 6h-7M18 6v7" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
// A billing document — folded corner + ruled lines. The instant "this is an
// invoice" mark on every queue card.
export function Invoice({ size = 14, ...p }: IconProps) {
  return (
    <svg {...base(size, p)} fill="none">
      <path
      ...
      <path d="M9 13h6M9 16.5h4M9.5 9.5h2" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
    </svg>
  );
}

// Rightward arrow — the "billed → catalog" comparison connector.
export function ArrowRight({ size = 12, ...p }: IconProps) {
  return (
    <svg {...base(size, p)} fill="none">
